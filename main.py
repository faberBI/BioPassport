...
def verify_passport_signature(passport, public_key_pem):
    """
    Verifica la firma digitale del passport.
    """
    signature_block = passport.get("signature")
    if not signature_block:
        return False
    try:
        public_key = serialization.load_pem_public_key(public_key_pem.encode())
        public_key.verify(
            bytes.fromhex(signature_block["signature"]),
            signature_block["hash"].encode(),
            padding.PKCS1v15(),
            hashes.SHA256()
        )
        # verifica integrità hash
        expected_hash = services.generate_hash(passport)
        return expected_hash == signature_block["hash"]
    except Exception:
        return False
...
if passport_id:
    ...
    # Mostra firma digitale se presente
    if passport.get("signature"):
        st.subheader("Firma digitale")
        st.write(f"Operatore: {passport['signature']['operator']}")
        st.text_area("Hash", passport['signature']['hash'], height=100)
        st.text_area("Signature (hex)", passport['signature']['signature'], height=100)
...
with tabs[0]:
    ...
    # --- Mostra PDF evidenziato
    if st.session_state.pdf_data and st.session_state.uploaded_pdf_bytes:
        st.subheader("PDF Prodotto evidenziato")
        highlighted_pdf_io = services.highlight_pdf_fields(BytesIO(st.session_state.uploaded_pdf_bytes), st.session_state.pdf_data)
        ...
        st.download_button("Scarica PDF Prodotto evidenziato", data=pdf_bytes, file_name="product_highlighted.pdf", mime="application/pdf")

    # --- Mostra certificati evidenziati
    if st.session_state.cert_data and st.session_state.uploaded_cert_bytes:
        ...
        for i, (cert_bytes, cert_fields) in enumerate(zip(st.session_state.uploaded_cert_bytes, st.session_state.cert_data)):
            ...
            """
            components.html(html_code, height=700)
            st.download_button(f"Scarica Certificato {i+1} evidenziato", data=pdf_bytes, file_name=f"cert_{i+1}_highlighted.pdf", mime="application/pdf")
...
with tabs[1]:
    if st.session_state.pdf_data and st.session_state.image_data:
        ...
        validated_pdf = {k: {"value": st.text_input(f"{k} (conf: {v.get('confidence',0)})", v.get("value",""), help=v.get("explanation","")), "confidence": v.get("confidence",0)} for k,v in st.session_state.pdf_data.items()}
        st.session_state.validated_pdf = validated_pdf

        st.subheader("Validazione dati Immagini")
        validated_img = {k: {"value": st.text_input(f"{k} (conf: {v.get('confidence',0)})", v.get("value",""), help=v.get("explanation","")), "confidence": v.get("confidence",0)} for k,v in st.session_state.image_data.items()}
...
# ======================================================
# TAB 3 — PUBBLICA (già riscritto con firma digitale)
# ======================================================
# Vedi sezione precedente con upload chiave privata e firma digitale
