    ...
    if st.session_state.pdf_data or st.session_state.image_data or st.session_state.cert_data:
        ...
        if st.session_state.pdf_data:
            for k,v in st.session_state.pdf_data.items():
                ...
                validated_pdf[k] = {"value": st.text_input(f"{k} (conf: {conf})", val, help=v.get("explanation","") if isinstance(v, dict) else ""), "confidence": conf}
        ...
        if st.session_state.image_data:
            for k,v in st.session_state.image_data.items():
                val = v.get("value","") if isinstance(v, dict) else str(v)
                conf = v.get("confidence",0) if isinstance(v, dict) else 0
                validated_img[k] = {"value": st.text_input(f"{k} (conf: {conf})", val, help=v.get("explanation","") if isinstance(v, dict) else ""), "confidence": conf}
...
with tabs[2]:
    if st.session_state.validated_pdf and st.session_state.validated_image:
        if st.button("Pubblica Digital Product Passport"):
            ...
            # Audit log pubblicazione
            log_audit(passport, user="operatore", action="publish", details=f"Pubblicato passport {pid}")
