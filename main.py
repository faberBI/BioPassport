...
with tabs[2]:
    ...
    if pp:
        ...
        # ==================================================
    # FIRMA ELETTRONICA SEMPLICE (SES/FES) – DEMO / TEST
    # ==================================================
    if ENABLE_SES:
        st.divider()
        ...
        # Inizializza stato solo la prima volta
        ...
        with st.form("ses_form", clear_on_submit=False):
            ...
            st.checkbox(
                "Consenti al firmatario di modificare dati",
                key="ses_allow_edit"
            )
        ...
        if submit_ses:
            with st.spinner("Invio richiesta di firma SES..."):
                try:
                    # ✅ Applica la firma SES sul PDF corretto
                    passport_file_path = services.save_passport_to_file(pp)  # salva il PDF
                    services.sign_passport_pdf_ses_openapi(
                        pdf_path=passport_file_path,
                    ...
                    # Aggiorna lo stato SES sul passport
                    services.espr_stamp(
                        pp,
                        actor="manufacturer",
        ...
        if ses_signed:
            ...
            st.download_button(
                ...
                data=buf,
                file_name=f"{pp.get('id', 'passport')}_qr_signed.png",
                mime="image/png"
            )
        else:
