...
def generate_passport_html(passport: dict, qr_base64: str = None) -> str:
    ...
    # Funzione per convertire immagini in base64 (assicurati che la tua funzione load_image_base64 esista)
    logo_base64 = load_image_base64("functions/logo_nuvia.jpeg")
    generated_at = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
    status = (passport.get("lifecycle") or {}).get("status", "")

    # --- Funzione semaforo ---
    def traffic_light(value: str) -> str:
        color_map = {
            "low": "#27CC6C",
            "medium": "#F6C23C",
            "high": "#3D0F06"
    ...
    # --- PASSPORT SUMMARY ---
    materials = passport.get("sections", {}).get("Materials", {})
    env_impact = passport.get("sections", {}).get("Environmental Impact", {})
    env_impact_val = env_impact.get("value","")
    env_impact_display = traffic_light(env_impact_val)

    materials_display = []
    for mat, info in materials.items():
        impact = info.get("impact","")
        materials_display.append(f"{traffic_light(impact)} {mat}")
    materials_display_html = "<br>".join(materials_display)
    ...
    <div class="section summary">
        ...
        <table class="data-table">
            ...
            <tr><td class='field-name'>Main Materials</td><td>{materials_display_html}</td></tr>
            <tr><td class='field-name'>Environmental Impact</td><td>{env_impact_display}</td></tr>
    ...
    for section_name, fields in (passport.get("sections") or {}).items():
        ...
        for k, v in fields.items():
            val = v.get("value") if isinstance(v, dict) else v
            if k.lower() in ["impact","environmental impact"]:
                val = traffic_light(val)
    ...
    <head>
        ...
        <style>
            ...
            .qr-inline {{ text-align:right; margin-bottom:15px; }}
            .qr-inline img {{ width:90px; border:1px solid #27CC6C; padding:5px; }}
