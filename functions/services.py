...
def generate_passport_html(passport: dict, qr_base64: str = None) -> str:
    ...
    cover_html = f"""
    <div class="cover">
        <img src="functions/logo_nuvia.jpeg" class="cover-logo"/>
        ...
        {f"<img class='cover-qr' src='data:image/png;base64,{qr_base64}' />" if qr_base64 else ""}

        <div class="cover-footer">
            Generated by Nuvia DPP System<br/>
            {generated_at}
        </div>
    </div>
    ...
    <div class="page-break"></div>
    <div class="about">
        <img src="functions/logo_nuvia.jpeg" class="about-logo"/>
        <h1>About Nuvia</h1>

        <p>Nuvia è un’azienda italiana specializzata in soluzioni digitali per la tracciabilità e sostenibilità dei prodotti.</p>

        <p>Supportiamo il Digital Product Passport secondo regolamento ESPR 2024/1781.</p>

        <h2>Mission</h2>
        <p>Abilitare trasparenza e sostenibilità lungo il ciclo di vita dei prodotti.</p>
    ...
    <head>
        ...
        <style>
            ...
            .cover-product {{ font-size: 20px; color: #555; }}
            .cover-meta {{ margin-top: 20px; font-size: 12px; }}
            .cover-qr {{ width: 140px; margin-top: 20px; }}

            /* QR INLINE */
            .qr-inline {{ text-align: right; margin-bottom: 10px; }}
            .qr-inline img {{ width: 90px; }}

            /* SECTIONS */
            .section {{ margin-bottom: 24px; }}
            h2 {{ border-bottom: 2px solid #2ECC71; padding-bottom: 4px; }}

            table {{ width: 100%; border-collapse: collapse; font-size: 11px; }}
            th, td {{ border: 1px solid #E5E7EB; padding: 6px; }}
            .field-name {{ font-weight: bold; background: #FAFAFA; width: 30%; }}

            pre {{ font-size: 9px; white-space: pre-wrap; }}

            /* IMAGES */
            .image-grid {{ display: flex; flex-wrap: wrap; gap: 10px; }}
            .image-card {{ width: 48%; border: 1px solid #E5E7EB; }}
            .image-card img {{ width: 100%; }}
            .caption {{ font-size: 10px; text-align: center; }}

            /* ABOUT / LEGAL */
            .about, .legal {{ padding: 40px; }}
